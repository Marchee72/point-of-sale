...
namespace PointOfSale.Business.Services
{
    public class ProductService : IProductService
    {
        ...
        public async Task<bool> EditCosts(string user, EditProductCosts newPrices)
        {
            try
            {
                foreach (var id in newPrices.ProductIds)
                {
                    var product_edit = await GetProductById(id);
                    product_edit.CostPrice += product_edit.CostPrice * (newPrices.IncreasePercentage / 100);
                    product_edit.Price += product_edit.Price * (newPrices.IncreasePercentage / 100);
                    product_edit.PriceWeb =+ product_edit.PriceWeb * (newPrices.IncreasePercentage / 100);
                    product_edit.PrecioFormatoWeb += product_edit.PrecioFormatoWeb * (newPrices.IncreasePercentage / 100);
                    product_edit.ModificationDate = DateTime.Today;
                    product_edit.ModificationUser = user;
                    bool response = await _repository.Edit(product_edit);

                    var queryProducts = await _repositoryListaPrecios.Query(p => p.IdProducto == id);
                    foreach (var queryProduct in queryProducts)
                    {
                        queryProduct.Precio += queryProduct.Precio * (newPrices.IncreasePercentage / 100);
                        var listaResult = await _repositoryListaPrecios.Edit(queryProduct);
                    }

                    if (!response)
                        throw new TaskCanceledException($"No se ha podido actualizar el producto con Id: {id}");
                }
                return true;
            }
            catch (Exception ex)
