...
function priceIncreasing() {
    ...
    var productosFiltrados = productos.filter(producto => idsProductos.includes(producto.idProduct));
    productosFiltrados.forEach(function (item) {
        var row = $('<tr></tr>');
        row.append('<td>' + item.description + '</td>');
        row.append('<td id="currentPrice_' + item.idProduct + '">' + item.costPrice.toFixed(2) + '</td>');
        row.append('<td id="updatedPrice_' + item.idProduct + '"></td>');
        $('#pricesTableBody').append(row);
    });

    $('#increasePercentage').on('keyup', function () {
        var increasePercentage = parseFloat($(this).val());
        productosFiltrados.forEach(function (item) {
            var updatedPrice = item.costPrice + (item.costPrice * (increasePercentage / 100));
            $('#updatedPrice_' + item.idProduct).text(updatedPrice.toFixed(2));
        });
    });
    $('#applyChanges').on('click', function () {
        const increasePercentage = parseFloat($('#increasePercentage').val());
        const products = [];

        productosFiltrados.forEach(function (item) {
            const updatedPrice = item.costPrice + (item.costPrice * (increasePercentage / 100));
        ...
        $.ajax({
            type: 'POST',
            url: '/inventory/UpdateProductCosts',
            contentType: 'application/json',
            data: JSON.stringify(data),
            success: function (response) {
                // Handle successful response (e.g., display success message, refresh table)
                console.log('Product costs updated successfully:', response);
            },
